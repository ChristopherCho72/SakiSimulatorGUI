...
class WeaponPage(tk.Frame):
    ...
    def __init__(self, master):
        ...
        for key in WeaponPage.GRID_POS.keys():
            ...
            for i in reversed(range(1, 5)):
                img = PIL.Image.open('data/img/weapon/%s%d.png'%(key, i)).resize(img_size, PIL.Image.ANTIALIAS)
        ...
        for key in WeaponPage.GRID_POS.keys():
            self.levels[key] = []
            for i in range(4):
                self.levels[key].append(tk.StringVar())
    ...
    def _render_level_slot(self):
        for key in WeaponPage.GRID_POS.keys():
            for i in range(4):
                ...
                entry.bind('<Return>', self._update_value)
                entry.bind('<FocusOut>', self._update_value)
    ...
    def _validate_level(self, action, index, value_if_allowed,
        ...
        only_digit = re.match(r"^[0-9]*$", value_if_allowed) is not None
        if not only_digit:
            return False
            ...
            for i in range(4):
                ...
                self.req_ups[key][i].set(
                    transform_english_amount_string(
                        calc_needed_upstone(key, level)
                    )
                )
                self.full_req_ups[key][i].set(
                    transform_english_amount_string(
                        calc_full_needed_upstone(key, level)
                    )
                )
